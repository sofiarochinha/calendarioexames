...
%	}
	...
	\nomenclature{GNS3}{Graphical Network Simulator 3}
	\nomenclature{HTTP}{Hypertext Transfer Protocol}
	\nomenclature{ACL}{Acess Control List}
	\nomenclature{TFTP}{Hypertext Transfer Protocol}
...
\end{itemize}
	...
	\label{seed}
		...
		No projeto as factories serviram para criar dados aleatórios para poder testar as funcionalidades testadas. No entanto à medida que estas foram implementadas as factories deixaram de ser úteis, estando no fim somente a \textit{factory} para criação de épocas.
		
		Para que esta factory funcione é preciso primeiro colocar todos os cursos e o tempo do exame a partir do seeder. Com isso foi criado um ficheiro que contém todos os cursos lecionados na Escola Superior e Gestão de Águeda tal como o exemplo apresentado no listing \ref{courseSeeder};
	...
	Com estas informações na base de dados a factory de épocas utiliza os ids de objetos aleatórios previamente criados a partir do \textit{model} correspondente, como se pode ver no listing \ref{epocasfactory}.
	...
	\begin{listing}[H]
		...
		\end{minted}
		\caption{\textit{Factory} para as épocas}
		\label{epocasfactory}
	\end{listing}
	...
	\subsection{Models}
		...
		Neste projeto foram criados vários \textit{models} em que cada um está relacionado a uma tabela da base de dados. Cada um tem todos os campos da tabela e também as relações entre as mesmas. Para uma chave estrangeira relacionar-se com uma tabela é necessário no \textit{model} colocar uma função em que retorna o \textit{model} da outra tabela, como está representado no listing \ref{epocasbelongsto}.
		...
		\end{minted}
		\caption{Relação \textit{belongsTo} para a tabela épocas}
		\label{epocasbelongsto}
	\end{listing}

	No entanto para que consigamos retorna a época - utilizando o caso acima descrito - é necessário no \textit{model} colocar a relação representada no listing \ref{calendarHasMany}. Neste caso como cada época pode ter mais que um calendário então é do tipo \textit{hasMany}. Se fosse somente um calendário para uma época então seria do tipo \textit{hasOne}.
		...
		\end{minted}
		\caption{Relação \textit{hasMany} para a tabela Calendar}
		\label{calendarHasMany}
	\end{listing}
	
	\section{Controllers}
	
	Depois da criação de todos os \textit{models} foi criado um \textit{controller} para cada página da aplicação tendo cada um várias funcionalidades para obter e/ou inserir dados na base de dados e posteriormente apresentar nas páginas. Os \textit{controllers} criados foram:
	
	\begin{itemize}
		\item LoginController - Gere todas as sessões na aplicação;
		\item ImportController - Importa o \textbf{.csv} para a base de dados;
		\item CalendarController - Obtém todos os dados da base de dados e envia para a view e adiciona os eventos na base de dados;
		\item ConfigurationsController - Obtém todos os dados sobre os docentes, salas, professores, epocas e cursos e envia para a view e também elimina as épocas na base de dados;
		\item CreateController - cria os calendários;
		\item ExportController - apresenta a view de exportar para pdf;
		\item HistoricController - apresenta a view de calendários anteriores;
	\end{itemize}

	\subsubsection{LoginController}
	
	Para a realização do login criou-se duas funcionalidades: o registo e o login. O registo foi criado para o cliente criar a sua conta e depois o registo será retirado. No entanto a partir de um formulário são inseridos o nome, email e uma palavra-passe que tem de ter no mínimo de 6 caracteres. Estes dados são validados com o código apresentado no listing \ref{registoValidação}.
		...
		\end{minted}
		\caption{Validação dos dados inseridos no formulário de registo}
		\label{registoValidação}
	\end{listing}
	
	A seguir é criado um novo utilizador na base de dados em que a palavra-passe fica encriptada com o método \textit{Hash}, como se pode ver no listing \ref{criarUser}, sendo a variável \$data um array com todos os dados.
	...
	\begin{listing}[H]
		...
		\end{minted}
		\caption{Criação de um novo utilizador}
		\label{criarUser}

	\end{listing}

	Depois de registo ter realizado é necessário verificar no login as credenciais e criar um sessão para que o utilizador possa aceder à aplicação. A partir de um formulário são inseridos o email e palavra-passe que serão verificados a partir do método Auth::attempt(\$credentials), tendo a variável \$credentials o email e password. Este método verifica se existe um utilizador com o email inserido e se existir comparada a hash da base de dados com a hash criada com a password inserida. Se forem iguais então uma sessão será criada e o utilizador entra na aplicação. E depois termina-se a sessão com o método Auth::logout(). 


	\subsubsection{ImportController}
	
	Depois de o utilizador ter iniciado sessão este terá de importar um csv que contém todos os dados necessários para a criação de calendários. Na view o csv importado é transformado num array com todos os dados em JSON que serão enviados por AJAX para o \textit{controller}. Neste transforma o JSON novamente em array com o método \textbf{json\_decode}. A seguir é utilizado o método \textit{createOrUpdate} que verifica o primeiro elemento se já existe na base de dados,  como é mostrado no listing \ref{profCreate}. Se existir faz \textit{update} do mesmo senão cria um novo elemento.
	...
	\begin{listing}[H]
		...
		\end{minted}
		\caption{Criação de um novo professor se não existir o número mecanográfico igual}
		\label{profCreate}
		
	\end{listing}

 	\subsubsection{CreateController}
 	
 	Assim que seja importado o \textbf{csv} o utilizador pode criar calendários. Para isso vão enviados para a \textit{view} variáveis com  essas informações (ver listing \ref{dadosCalendar}). Neste caso os dados já o ano de curso são ordenados pelo ano e os cursos só são apresentados uma vez - utilizando o método Course::distintc('course\_code) - ignorando cursos repetidos. 
	...
	\begin{listing}[H]
		...
		return view('create_calendar', compact(["courses",
		"evaluation_season","coursesall"]));
	...
	Depois de o utilizador escolher qual calendário será criado e caso não exista um igual - com a mesma época, curso e ano de curso, como se pode ver no listing \ref{criarcalendárioController} - então será adicionado à base de dados. Os dados sobre o calendário são enviados por AJAX na view.
	...
	\begin{listing}[H]
		...
		\end{minted}
		\caption{Criação de um novo calendário caso não exista}
		\label{criarcalendárioController}
		
	\end{listing}

	\subsubsection{CalendarController}
	
	Com este \textit{controller} é possível marcar o exames no calendário e guardar na base de dados. E também se existir alguma alteração será guardada. Os dados são enviadas por AJAX para o \textit{controller}. 
	
	Primeiro será verificado se existe alguma exame marcado para aquela disciplina com o método apresentado no listing \ref{evaluationslot}. Caso retorne ``null" cria um novo exame, caso contrário atualiza o mesmo.
	
	\begin{listing}[H]
		\begin{minted}{php}
		 $evaluationSlot = EvaluationSlot::where('subject',
		 $subject)->where('calendar_id', $calendar)->first();
		\end{minted}
		\caption{Criação de um novo calendário caso não exista}
		\label{evaluationslot}
	...
	A primeira versão da aplicação reúne as funcionalidades básicas que foram inicialmente apresentadas nos protótipos em \textit{wireframes}. 
	Esta versão tem presente funcionalidades básicas como a página de inicio de sessão, que valida um utilizador com recurso à base de dados, navegação para a página de criação de um calendário onde é pedido para escolher quais os cursos a colocar no projeto assim como os anos letivos desse curso que se pretende colocar no calendário e ainda escolher as épocas de avaliação para adicionar também ao projeto. 
	
	Os dados da base de dados usados para construir o calendário, como por exemplo os docentes, as unidades curriculares e salas, podem ser previamente inseridos na base de dados com recurso a um ficheiro \textbf{csv} que costuma ser disponibilizado pela ESTGA para a preparação dos calendários. 
	Nesta página o utilizador escolhe o ficheiro a importar, é apresentado uma pré visualização dos dados que estão presentes no ficheiro e para que tabelas serão importados e por fim é apresentado o botão para finalizar a importação. 
	Em ocasião de não haver um ficheiro \textbf{csv} importado o utilizador é advertido por uma mensagem de aviso amarelo  em estilo \textit{popup} de que este não foi importado, o inverso também acontece no caso de já ter sido importado mas agora com uma mensagem de sucesso em cor verde. 
	Estas mesmas mensagens também aparecem na página de criação do calendário para advertir o utilizador da criação do mesmo sem ter dados importados. 
	Esta funcionalidade foi conseguida recorrendo ao uso de \textit{cookies} no \textit{browser} que ficam guardados no lado do cliente e implementada com \textit{javascript}. 
	
	Os dados importados para a base de dados referentes à informação necessária para a construção dos calendários pode ser consultada e editada em páginas dedicadas com \textit{tabs} para cada uma das funcionalidades. 
	Após estar terminado um projeto, este pode ser exportado e apagado. 
	Este ato apaga a base de dados para que um projeto novo possa ser começado de início e desativa o \textit{cookie} previamente colocado para que os avisos voltem ao estado inicial. 
	
	\chapter{Testes com potenciais clientes}
	Para testar a aplicação foram feitos testes com potenciais utilizadores para avaliar as nossas funcionalidades realizadas nesta primeira versão da aplicação. Como nem todas as funcionalidades estavam implementadas então foram testadas algumas com as seguintes tarefas:
	
	\begin{itemize}
		\item Login
		\item Importar ficheiro \textbf{csv};
		\item Visualizar informações sobre as disciplinas, salas e docentes que foram importadas;
		\item Criar um calendário para um curso, ano de curso e uma época;
		\item Marcar um exame de um curso numa data 
		\item Fazer \textit{Log Out}.
	\end{itemize}
	...
	No final da secção foram anotadas todas as alterações que terão de ser realizadas para a última versão da aplicação estando dividas por categorias - calendário atual, configurações e criar calendário. 
	
	
	\begin{center}
		\label{analisederesultados3}
		\begin{longtable}{|m{2.2cm}|m{12cm}|}
			\caption{Análise de resultados da terceira sessão de avaliação}\\
			
			\hline			
			\textbf{Categoria}&\textbf{Descrição da alteração} \\
			\hline
			
			Configurações& Poder adicionar docentes, disiciplinas, salas e épocas\\
			
			\hline
			\multirow{3}{2cm}{Calendário atual}& Colocação do número de salas utilizadas nos exames marcados \\
			&Colocação do número de professores no títulos dos exames marcados\\
			&Colocação de incompatibilidades na marcação de exames\\
			
			\hline
		
			Criar calendário & Colocação os nomes por ordem alfabética\\
			\hline
		\end{longtable}
	\end{center}
	...
	\chapter{Análise de resultados}
		...
		Por fim foi feita uma análise de todo o projeto em que se pode verificar quais requisitos inicialmente propostos foram realizados. Os implementados foram a importação de um ficheiro .csv para a base de dados, foi a marcação dos exames numa época, criação de calendário e visualização dos dados importados, ter uma \textit{preview} dos mesmos, autenticação. Todos os outros não foram implementados tendo a aplicação funcionalidades básicas.
	...
	\section{Alocação da aplicação no servidor}
		...
		Para alocação do servidor utilizou-se da aplicação ``filezilla'' para colocação dos ficheiros no servidor. No entanto para a base de dados teve-se de mudar o ficheiro .env para que possa aceder à base de dados do servidor, tal como está no listing \ref{ficheiroenv}.
	
		\begin{listing}[H]
			\begin{minted}{php}
				DB_CONNECTION=pgsql
				DB_HOST=estga-dev.clients.ua.pt
				DB_PORT=5432
				DB_DATABASE=ptdw-2021-gr4.
				DB_USERNAME=ptdw-2021-gr4.
				DB_PASSWORD=qYgQuY?2VFw)3,Xz
			\end{minted}
			\caption{Seeder para a tabela ``time\_slot''}
			\label{ficheiroenv}
		\end{listing}
	
	Com acesso ao servidor por ssh utilizou-se o comando \textbf{php artisan migrate:fresh --seed} para poder fazer seed, como mostrado na secção \ref{seed}, para poder criar as tabelas na base de dados e colocar todos os cursos e momentos do dia para avaliação.
	
	
	\chapter{Conclusão}
	
	Neste projeto temático foi proposta e implementada uma \textit{web app} parcialmente funcional que permite ao utilizador criar uma calendário de eventos de avaliações académicas em determinadas épocas à escolha do mesmo com recurso a ficheiros \textbf{csv} auxiliares para pré preencher as informações sobre salas, docentes, cadeiras, cursos e anos de curso.
	
	O projeto foi desenvolvido com uma metodologia de comunicações regulares com o cliente onde ideias eram apresentadas e alteradas conforme as necessidades deste e ainda foram feitos testes públicos de usabilidade para obter \textit{feedback} de utilizadores.
	Foi implementada uma base de dados escrita com a tecnologia PostgresSQL alojada no mesmo servidor que serve a aplicação de criação de calendários servindo esta para guardar projetos em construção assim como um histórico de calendários anteriores.
	...
	Devido a alguns atrasos causados por problemas técnicos com o \textit{template} escolhido, com imprevistos, mudanças na estrutura do grupo e ainda a ambição do projeto, ainda existem funcionalidades da aplicação por implementar que lamentavelmente o tempo disponível não permitiu.
