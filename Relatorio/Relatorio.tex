...
\end{itemize}
	...
	\subsection{Base de dados - \textit{seeder}}
		...
		No projeto as \textit{factories} serviram para criar dados aleatórios para poder testar as funcionalidades testadas. No entanto à medida que as funcionalidades foram implementadas as \textit{factories} deixaram de ser úteis, estando no fim somente a \textit{factory} para a criação de épocas.
		
		Para que esta factory funcione é preciso primeiro colocar todos os cursos e o tempo do exame a partir do \textit{seeder}. Com isso foi criado um ficheiro que contém todos os cursos lecionados na Escola Superior e Gestão de Águeda tal como o exemplo apresentado no listing \ref{courseSeeder};
	...
	Esta versão tem presente funcionalidades básicas como a página de inicio de sessão, que valida um utilizador com recurso à base de dados, navegação para a página de criação de um calendário onde é pedido para escolher quais os cursos a colocar no projecto assim como os anos letivos desse curso que se pretende colocar no calendário e ainda escolher as épocas de avaliação para adicionar também ao projecto.
	...
	Após estar terminado um projecto, este pode ser exportado e apagado.
	Este acto apaga a base de dados para que um projecto novo possa ser começado de início e desativa o \textit{cookie} previamente colocado para que os avisos voltem ao estado inicial.
